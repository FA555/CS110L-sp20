...
impl Process {
    ...
    pub fn list_fds(&self) -> Option<Vec<usize>> {
        ...
        for fd in fs::read_dir(dir).ok()? {
            let fd = fd.ok()?.file_name().to_str()?.parse().ok()?;
            fds.push(fd);
        }
        Some(fds)
    }
    ...
    pub fn print(&self) {
        println!("\n========== \"{}\" (pid {}, ppid {}) ==========", self.command, self.pid, self.ppid);
        ...
        for child_process in get_child_processes(self.pid).unwrap() {
            child_process.print();
        }
    }
}
